            ...
            try:
                ...
                for art_data, (rewritten_data, failure_reason) in zip(batch, batch_results):
                    try:
                        ...
                        if wp_post_id:
                            # After publishing, attempt a robust sanitation that covers content and excerpt,
                            # with retries and aggressive fallbacks encapsulated in the WordPress client.
                            try:
                                sanitized_ok = wp_client.sanitize_published_post(wp_post_id)
                                if sanitized_ok:
                                    logger.info(f"✅ Post {wp_post_id} published and sanitized if CTA present.")
                                else:
                            ...
                            # Aguardar entre publicações para evitar sobrecarga (garante SEO perfeito)
